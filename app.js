...
const DEFAULT_HABITS_GOOD = [
  { id: "hg0", name: "Ставане 06:00" }, { id: "hg1", name: "Развиване на умение" },
...
function migrateReading(st) {
  ...
  const wh = (st.habitsGood || []).find((x) => x.id === "hg0");
  if (wh && /05:30/.test(wh.name)) wh.name = "Ставане 06:00"; // новият режим: ставане в 06:00 всеки ден
  return st;
}
function loadState() {
  const empty = { checks:{}, refl:{}, tasks:{}, weights:{}, shopping:{}, shopExtra:{}, shopEdits:{}, planEdits:{}, edits:{}, reading:{}, shopWife:false,
...
function save() {
  ...
  saveTimer = setTimeout(() => {
    ...
    try { localStorage.setItem(LS_KEY, JSON.stringify(state)); setSync(cloudOn ? "облак…" : "запазено"); }
...
function setReadPages(key, val) {
  ...
  r[key] = val; // пазим и 0 (не трием ключа) — иначе облачният merge може да върне старата стойност
  state.reading = r;
  save();
}
...
function viewToday() {
  ...
  const isCardio = w && /кардио/i.test(w.t);
  const items = [];
  items.push({ id:"wake", time:"06:00", t:"Ставане + вода", s:"Телефонът остава далеч първите 30 мин", k:"habit" });
  if (workday) {
  ...
  items.push({ id:"evening", time:"21:30", t:"Вечерна рутина + време със семейството", s:"Телефон извън спалнята · лягане 22:15–22:30", k:"habit" });
  ...
  const sec = h("section", null,
    ...
    h("div",{class:"tl"},
      items.map((e) => {
        ...
        if (e.k==="info")
          return h("div",{class:"tlInfo","data-time":e.time}, h("span",{class:"tlTime"},e.time), h("span",null,e.t,e.s?h("i",{class:"tlInfoS"}," · "+e.s):null),
...
/* ---------- ПЛАН (под-табове: Хранене / Спорт) ---------- */
function viewPlan() {
  const subs = [["food","Хранене"],["train","Спорт"]];
  return h("section",null,
    ...
    planSub==="train" ? planTrain() : planFood());
}
...
function viewProgress() {
  ...
  return h("section",null,
    ...
    exportNudge(),
    h("p",{class:"secS"}, cloudOn
      ? "Данните се синхронизират автоматично между устройствата ти през облака (и се пазят локално за офлайн). Експортът остава като допълнителна застраховка."
      : "Всичко се пази локално на това устройство. Свали резервно копие или прехвърли на друго устройство."),
  ...
  inp.addEventListener("change",()=>{
    ...
    rd.onload=()=>{
      try{
        ...
        state={checks:s.checks||{},refl:s.refl||{},tasks:s.tasks||{},weights:s.weights||{},shopping:s.shopping||{},shopExtra:s.shopExtra||{},shopEdits:s.shopEdits||{},planEdits:s.planEdits||{},edits:s.edits||{},reading:s.reading||{},shopWife:!!s.shopWife,
...
const shopGroups = (list, tripId) => (list||[]).filter((g) => (g.trip||0) === tripId);
function shopCard(groups, shopChecks, toggleShop, mKey) {
  const edits = state.shopEdits[mKey]||{};
  return h("div",{class:"card"},
    groups.map((grp)=>[
      ...
      grp.items.map((it,i)=>{
        ...
        const text = edits[id] !== undefined ? edits[id] : it;
...
// редакция на продукт от месечния списък — корекцията се пази върху оригинала
function shopItemForm(mKey, id, text, orig) {
  const inp = h("input",{class:"inp",value:text,"aria-label":"Текст на продукта"});
  const saveIt = () => {
    const t = inp.value.trim(); if (!t) return;
    const m = { ...(state.shopEdits[mKey]||{}) };
    if (t === orig) delete m[id]; else m[id] = t;
    state.shopEdits = { ...state.shopEdits, [mKey]: m };
    editingShopItem = null; save(); render();
  };
  inp.addEventListener("keydown",(e)=>e.key==="Enter"&&saveIt());
  return h("div",{class:"addForm",style:"margin:6px 0"},inp,
    h("button",{class:"btn",onclick:saveIt},"Запази"),
    text!==orig?h("button",{class:"btnGhost",onclick:()=>{
      const m={...(state.shopEdits[mKey]||{})}; delete m[id];
      state.shopEdits={...state.shopEdits,[mKey]:m};
      editingShopItem=null; save(); render();
    }},"Оригинал"):null,
...
function viewShop() {
  ...
  // при включена отметка се показва общият списък (твоят + нейният, със събрани количества)
  const hasWife = !!m.shoppingCombined;
  const wifeOn = hasWife && !!state.shopWife;
  const list = wifeOn ? m.shoppingCombined : m.shopping;
  const staples = shopGroups(list,0);
  return h("section",null,
    ...
    hasWife ? h("div",{class:"card wifeToggle"},
      checkBtn(wifeOn,()=>{ state.shopWife=!state.shopWife; save(); render(); },true),
      h("div",{class:"wifeTxt"},
        h("strong",null,"+ Нейният списък"),
        h("span",null, wifeOn
          ? "Показани са ОБЩИТЕ количества за двама ви (нейните добавки са отбелязани)."
          : "Отметни, за да видиш общите количества и грамажи за двама ви."))) : null,
    SHOP_TRIPS.map((trip) => {
      const groups = shopGroups(list, trip.id);
      if (!groups.length) return null;
      return h("div",null,
        h("h3",{class:"shopTripT"},trip.label),
        h("p",{class:"secS"},trip.hint),
        shopCard(groups, shopChecks, toggleShop, mKey));
    }),
    staples.length ? h("div",null,
      h("h3",{class:"shopTripT"},"По всяко време ",h("span",{class:"secHint"},"веднъж месечно")),
      shopCard(staples, shopChecks, toggleShop, mKey)) : null,
    shopExtraSection(shopKey, extra));
}
...
function render() {
    ...
    h("div",{class:"hdrTop"},
      h("div",{class:"brand"},
        ...
        h("div",{class:"brandCopy"},
          ...
          h("span",{class:"brandKanji",lang:"ja"},"継続は力なり"),
  ...
  const nav = h("nav",{class:"nav","aria-label":"Основна навигация"},
    [["today","flame","Днес","今日"],["plan","bowl","План","計画"],["shop","cart","Пазар","買物"],["calendar","cal","Календар","暦"],["habits","habit","Навици","習慣"],["progress","chart","Прогрес","進歩"]]
      .map(([id,ic,lbl,jp])=>h("button",{class:`navTab ${tab===id?"act":""}`,"data-tab":id,"aria-label":lbl,"aria-current":tab===id?"page":null,
        ...
        h("span",{class:"navIcon"},icon(ic,20)),h("span",{class:"navLabel"},lbl),h("small",{class:"navJp",lang:"ja"},jp))));
...
/* ============================================================
   ОБЛАЧНА СИНХРОНИЗАЦИЯ (Firebase Firestore)
   - localStorage остава основен (офлайн + мигновено зареждане)
   - Firestore пази 1 документ (zharava/state) и го синхронизира
     в реално време между телефона и лаптопа
   - Ако конфигът е празен или няма интернет → приложението
     работи както досега, само локално
   ============================================================ */
// ▼▼▼ ПОСТАВИ ТУК КОНФИГА ОТ FIREBASE CONSOLE (Project settings → Your apps → Web) ▼▼▼
const FB_CONFIG = {
...
// но записи, които има само едната страна, не се губят.
function mergeStates(a, b) {
  const pair = (a.updatedAt || 0) <= (b.updatedAt || 0) ? [a, b] : [b, a];
  const older = pair[0], newer = pair[1];
  const out = { ...older, ...newer };
  for (const f of MAP_FIELDS) out[f] = { ...(older[f] || {}), ...(newer[f] || {}) };
  out.habitsGood = (newer.habitsGood && newer.habitsGood.length) ? newer.habitsGood : older.habitsGood;
  out.habitsBad  = (newer.habitsBad  && newer.habitsBad.length)  ? newer.habitsBad  : older.habitsBad;
  return out;
}
const stateSig = (s) => { const c = { ...s }; delete c.updatedAt; return JSON.stringify(c); };

function cloudPush(force) {
  if (!cloudDb) return;
  clearTimeout(cloudPushTimer);
  cloudPushTimer = setTimeout(() => {
    cloudDb.collection(CLOUD_COL).doc(CLOUD_DOC).set(state)
      .then(() => setSync("облак ✓"))
      .catch(() => setSync("само локално"));
  }, force ? 0 : 800);
}

function initCloud() {
  if (typeof firebase === "undefined" || !FB_CONFIG.apiKey || !FB_CONFIG.projectId) return;
  try {
    firebase.initializeApp(FB_CONFIG);
    cloudDb = firebase.firestore();
    cloudOn = true;
    cloudDb.collection(CLOUD_COL).doc(CLOUD_DOC).onSnapshot((snap) => {
      if (snap.metadata.hasPendingWrites) return;      // ехо от собствен запис
      if (!snap.exists) { cloudPush(true); return; }   // първо устройство → качва данните
      const remote = snap.data() || {};
      const merged = migrateReading(mergeStates(state, remote));
      const changedLocal = stateSig(merged) !== stateSig(state);
      const changedRemote = stateSig(merged) !== stateSig(remote);
      if (changedLocal) {
        state = merged;
